...
<form name="print" id="print" method="post" target="_blank" >
...
yepnope({
	load : [       
		...
		//SWFObject
		"${ctx}/resources/extjs/ux/swfobject.js"
		],
	complete : function() {
...
message = {
	...
	print:ctx + "/message/print" ,//Lodop打印
	swf:ctx + "/message/swf" ,//swf显示
	read:ctx + "/message/read/" ,//消息已阅
...
message.selModel = new Ext.grid.CheckboxSelectionModel({
			...
			listeners : {
				'rowselect' : function(selectionModel, rowIndex, record) {
					...
					message.printAction.enable();
					message.swfAction.enable();
				},
...
message.colModel = new Ext.ux.grid.LockingColumnModel({
			...
			columns : [message.selModel,new Ext.grid.RowNumberer({locked: true}), {
						...
						dataIndex : 'mxdx',
						renderer:function (value, cellmeta, record, rowIndex, columnIndex, store){							
							message.MultiSelect.setValue(value);
							return message.MultiSelect.getCheckedDisplay();						
						}
					}, {
						header : '发布时间',
...
message.editAction = new Ext.Action({
		...
		//		message.MultiSelect.setValue(record.get('mxdx'));
...
message.viewAction = new Ext.Action({
			...
			handler : function() {
				...
				if(record.get('isread')==0)
					message.readFun(record.get('id')); //消息已阅
			}
		});		
/** 删除 */
...
/** Lodop 打印 */
message.printAction = new Ext.Action({
			text : '输出打印-Lodop',
			iconCls : 'printer',
			disabled : true,
			handler : function() {
...
/** swf 打印 */
message.swfAction = new Ext.Action({
			text : '输出打印-SWF',
			iconCls : 'printer',
			disabled : true,
			handler : function() {
...
message.MultiSelect = new Ext.ux.form.LovCombo({
  name:"mxdx",
  hiddenName:"mxdx",
  fieldLabel:"面向对象",
  store:new Ext.data.ArrayStore({
			fields : ['v', 't'],
			data : Share.map2Ary(message.role)
		}),
  mode:'local',
  triggerAction:'all',
  allowBlank:true,
...
message.alwaysFun = function() {
	...
	message.printAction.disable();
	message.swfAction.disable();
...
//	alert(message.MultiSelect.getValue());
	// 发送请求
	Share.AjaxRequest({
				url : message.save,
...
//已阅消息处理
message.readFun = function(ids){
	Share.AjaxRequest({
		url : message.read +ids ,
		showMsg:false,
   		showWaiting:false,
		callback : function(json) {
			message.alwaysFun();
...
message.printFun =function(){
	var record = message.grid.getSelectionModel().getSelected();	
	var id = record.get('id');
	print.action=message.print;
	Ext.getDom("print").id.value = id;
	Ext.getDom("print").action=message.print;
	Ext.getDom("print").submit();	
};

message.swfFun =function(){
